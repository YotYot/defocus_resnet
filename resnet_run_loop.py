...
def resnet_main(
    flags_obj, config, conf_matrix, model_function, input_function, dataset_name, shape=None):
  ...
  def input_fn_predict():
    return input_function(
        ...
        batch_size=distribution_utils.per_device_batch_size(
            flags_obj.batch_size, flags_core.get_num_gpus(flags_obj)),
        num_epochs=1, get_one_item=False, conf_matrix=conf_matrix)

  total_training_cycle = (flags_obj.train_epochs //
                          flags_obj.epochs_between_evals)

  if config._mode == 'predict':
      results = classifier.predict(input_fn_predict)
      results = list(results)
      for result in results:
        conf_matrix.predictions.append(result['classes'])
  elif config._mode == 'eval':
